 ...
 *
 * $gameTimers.isTimeElapsedOverThanSec(name, seconds); and $gameTimers.isTimeElapsedOverThanMinutes(name, minutes); does the same thing with seconds and minutes, respectively.
 */
...
FLZ_Timers.prototype.isTimeElapsedOverThanSec = function(name, seconds) {
	return Date.now() - this.timers[name] >= seconds * 1000;
}

FLZ_Timers.prototype.isTimeElapsedOverThanMinutes = function(name, minutes) {
	return Date.now() - this.timers[name] >= minutes * 60000;
}
